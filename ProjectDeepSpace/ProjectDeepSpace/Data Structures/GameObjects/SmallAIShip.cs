...
{
    public class SmallAIShip : GameObject
    {
        public SmallAIShip(int ID, Vector3 loc, Vector2 hexLoc)
        {
            iD = ID;
            team = Player.AI;
            type = GameObjectType.Small_AI_Ship;
            ...
            myWeapon = new CustomWeapon("Short Range Anti-Matter Beam", 5, 2, 5, 0.4f, WeaponType.Laser);
            myEngine = new CustomEngine("Simple BioMechanical Conversion Engine", 2);
        }

        public override void LoadModel(ContentManager content, BasicEffect effect)
        {
            myModel = content.Load<Model>("Models/Enemy_Small");
        ...
        public override void DrawModel(ContentManager content, BasicEffect effect, ref GlobalVariables globalVariables)
        {
            foreach (ModelMesh mesh in myModel.Meshes)
            {
                ...
                {
                    currentEffect.World = 
                        Matrix.CreateScale(0.0015f, 0.0015f, 0.0015f) *
                        Matrix.CreateFromQuaternion(rot) *
                        Matrix.CreateTranslation(loc + new Vector3(0, 0f, 0.1f));
