...
{
    public class MediumAIShip : GameObject
    {
        public MediumAIShip(int ID, Vector3 loc, Vector2 hexLoc)
        {
            iD = ID;
            team = Player.AI;
            type = GameObjectType.Medium_AI_Ship;
            ...
            myWeapon = new CustomWeapon("Medium Range Anti-Matter Beam", 5, 3, 8, 0.6f, WeaponType.Laser); 
            myEngine = new CustomEngine("Advanced BioMechanical Conversion Engine", 10);
        }

        public override void LoadModel(ContentManager content, BasicEffect effect)
        {
            myModel = content.Load<Model>("Models/Enemy_Mid");
        ...
        public override void DrawModel(ContentManager content, BasicEffect effect, ref GlobalVariables globalVariables)
        {
            foreach (ModelMesh mesh in myModel.Meshes)
            {
                ...
                {
                    currentEffect.World =
                        Matrix.CreateScale(0.0013f, 0.0013f, 0.0013f) *
